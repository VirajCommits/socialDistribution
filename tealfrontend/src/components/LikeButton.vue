...
export default {
  ...
  methods: {
    async fetchLikes() {
      try {
        const apiUrl = `${process.env.VUE_APP_API_BASE_URL}/authors/${this.authorId}/posts/${this.postId}/likes/`;
    ...
    async toggleLike() {
      try {
        const apiUrl = `${process.env.VUE_APP_API_BASE_URL}/authors/${this.authorId}/posts/${this.postId}/like/`;
        if (this.liked) {
